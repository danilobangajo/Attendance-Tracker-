...
function switchDepartment(dept) {
    ...
    // Refresh weekly report for the new department if modal is open
...
function viewEmployeeDetails(employeeName) {
    ...
    filteredRecords.forEach(r => {
        ...
        if (r.reason) {
            ...
            reasonBtn.className = 'btn btn-sm btn-outline-warning';
            reasonBtn.title = 'View Reason';
            reasonBtn.innerHTML = '<i class="bi bi-chat-left-text"></i>';
            reasonBtn.addEventListener('click', () => showReasonModal(r.reason, formattedDate));
...
function toggleAdminView() {
    ...
    if (isOpen) {
        ...
        document.getElementById('formCol').className = 'col-lg-4 offset-lg-4';
        const btn = document.getElementById('adminToggleBtn');
        btn.innerHTML = '<i class="bi bi-shield-lock me-1"></i>Admin';
        btn.classList.remove('btn-secondary');
        btn.classList.add('btn-outline-secondary');
        // Move tab buttons back to center
        const tabBar = document.getElementById('tabBar');
        const tabBtnGroup = tabBar.querySelector('.btn-group');
        tabBtnGroup.className = 'btn-group position-absolute start-50 translate-middle-x';
        document.getElementById('attendanceForm').scrollIntoView({ behavior: 'smooth' });
    } else {
...
function verifyAdminPassword() {
    ...
    if (input.value === 'COMSADMIN2026!') {
        ...
        // Move tab buttons to the left side
        const tabBar = document.getElementById('tabBar');
        const tabBtnGroup = tabBar.querySelector('.btn-group');
        tabBtnGroup.className = 'btn-group me-auto';
    } else {
